	...
	return (
		<div className="flex flex-col text-blue-950">
			...
			<div className="flex flex-row items-center">
				{isVisible ? (
					<IoIosArrowDropupCircle
						className="aspect-square h-6 w-6 transform hover:scale-105 hover:opacity-75"
			...
			>
				...
				<div className="modifyConfigSelect flex flex-row flex-wrap items-center justify-start gap-3">
					...
					{selectedOption === 'any' && (
						<input
							...
							className="border-gray-30 rounded-md border bg-gray-100 px-3 py-2 text-blue-950 placeholder:text-gray-500"
						/>
					)}
				...
				<p className="mb-4">
					...
					<span className="font-mono">'{currentOwner}'</span> (e.g.,
					using Electrum) with an amount of 0:
				</p>
				...
				{selectedOption === 'owner' && (
					<p className="mb-3 rounded-md bg-yellow-300 p-2 text-black">
						<b>Warning: </b>
						Changing the owner transfers control of '{queryString}'
						to another address. Please double-check the new owner
						address, as this action cannot be undone.
						<br />
				...
				<p>
					The script value is encoded in hexadecimal. In ASCII, it
					reads: '{RAW_ASCII_ScriptValue}'
				</p>
			</div>
		</div>
	);
